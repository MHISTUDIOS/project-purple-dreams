...
function Download-And-Run-Installer {
    ...
    try {
        ...
        Write-Host "[OK] $($Name) installer has exited." -ForegroundColor Green
    } catch {
        Write-Host "[ERROR] Failed to launch installer for $($Name):`n    $($_.Exception.Message)" -ForegroundColor Red
    }
    Pause-ForKey
}
